          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-start">
            ...
            <div className="lg:col-span-5 lg:sticky lg:top-32 h-auto flex flex-col gap-10">
              ...
              <motion.div 
                ...
                className="relative"
              >
                <div className="relative rounded-[40px] overflow-hidden group shadow-2xl skew-x-1">
                  <img 
                    src="https://images.unsplash.com/photo-1541534741688-6078c64b5903?auto=format&fit=crop&q=80&w=1200" 
                    alt="Gestão Esportiva" 
                    className="w-full h-[500px] object-cover grayscale hover:grayscale-0 transition-all duration-1000"
                  />
                  <div className="absolute inset-0 bg-lime-500/10 group-hover:bg-transparent transition-colors duration-500" />
                  
                  {/* Green Glow Effect */}
                  <div className="absolute -inset-1 bg-lime-400 blur-2xl opacity-10 group-hover:opacity-20 transition-opacity duration-500 -z-10" />
                </div>
              </motion.div>
            </div>
              ...
              <div className="relative group">
                <div className="flex overflow-x-auto snap-x snap-mandatory gap-6 pb-12 scrollbar-none no-scrollbar">
                  {categorySolutions.map((category, catIdx) => (
                    ...
                    >
                      <div className="p-8 rounded-[40px] bg-white border border-zinc-200 h-full shadow-sm">
                        <div className="mb-8">
                          <h3 className="text-xl font-black text-zinc-900 mb-2 uppercase tracking-widest">{category.category}</h3>
                          <p className="text-zinc-500 font-light text-sm">{category.description}</p>
                        </div>
                        ...
                        <div className="flex flex-col gap-4">
                          {category.items.map((item, idx) => (
                            <div
                              key={idx}
                              className="px-6 py-4 rounded-2xl bg-zinc-50 border border-transparent hover:border-lime-500/30 hover:bg-white hover:shadow-lg transition-all group flex gap-4 items-center"
                            >
                              <div className="w-10 h-10 rounded-xl bg-white flex items-center justify-center p-2 shadow-sm group-hover:bg-lime-50 transition-colors">
                                <item.icon className="w-full h-full text-zinc-400 group-hover:text-lime-500 transition-colors" strokeWidth={2} />
                              </div>
                              <h4 className="text-sm font-bold text-zinc-900 group-hover:text-lime-600 transition-colors">{item.title}</h4>
                            </div>
                          ))}
                        </div>
                      </div>
...
export default function App() {
  ...
  return (
    <div className={`min-h-screen transition-colors duration-500 ${currentPage === 'home' ? 'bg-zinc-950 text-white' : 'bg-[#f8f9fa] text-zinc-900'}`}>
            ...
            <div className="text-left">
              ...
              >
                ...
                <button 
                  ...
                  className="px-8 py-4 bg-lime-400 text-zinc-950 rounded-full font-black text-base hover:bg-lime-300 shadow-[0_0_25px_-5px_rgba(190,242,100,0.6)] hover:scale-105 transition-all flex items-center gap-2 group"
                >
      ...
      <section id="dashboard" className="py-32 px-6 relative overflow-hidden bg-zinc-950">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-8 items-center">
            
            {/* Left Column: Light Mode Tilted Mockup */}
            <motion.div
              initial={{ opacity: 0, x: -50, rotateY: 35, rotateX: 15, scale: 1.4 }}
              whileInView={{ opacity: 1, x: 0, rotateY: 35, rotateX: 15, scale: 1.4 }}
              viewport={{ once: true }}
              transition={{ duration: 1.2, ease: "easeOut" }}
              className="relative hidden lg:block perspective-1000 lg:col-span-1 z-0 origin-right"
            >
              <div className="rounded-2xl overflow-hidden border border-black/5 shadow-2xl bg-white aspect-video relative group transition-transform duration-500 hover:scale-105">
                {/* Mock Light UI UI */}
                ...
                <div className="p-6 bg-white h-full">
                  <div className="flex gap-4 mb-6">
                    <div className="h-16 flex-1 rounded-xl bg-zinc-100/80 animate-pulse" />
                    <div className="h-16 flex-1 rounded-xl bg-zinc-100/80 animate-pulse" />
                  </div>
                  <div className="h-32 rounded-xl bg-zinc-50/50 animate-pulse w-full border border-dashed border-zinc-200" />
                </div>
                <div className="absolute inset-0 bg-gradient-to-tr from-white/0 via-white/40 to-white/0 pointer-events-none" />
                <div className="absolute bottom-4 left-4 glass-light px-4 py-1.5 rounded-full text-[10px] font-black text-zinc-800 tracking-tighter uppercase">MODO CLARO</div>
              </div>
            </motion.div>
            ...
            >
              <div className="mb-16 text-center">
                <p className="text-lime-400 font-bold uppercase tracking-widest text-xs mb-6 px-4 py-1 rounded-full bg-lime-400/10 border border-lime-400/20 inline-block">Assista a uma demonstração</p>
                <h3 className="text-5xl md:text-6xl font-bold bg-clip-text text-transparent bg-gradient-to-b from-white to-zinc-500 tracking-tighter">O futuro da sua gestão chegou</h3>
              </div>
              <div className="relative rounded-[3.5rem] p-5 overflow-hidden border border-white/10 shadow-[0_0_100px_rgba(190,242,100,0.25)] bg-zinc-900 group">
            ...
            {/* Right Column: Dark Mode Tilted Mockup */}
            <motion.div
              initial={{ opacity: 0, x: 50, rotateY: -35, rotateX: 15, scale: 1.4 }}
              whileInView={{ opacity: 1, x: 0, rotateY: -35, rotateX: 15, scale: 1.4 }}
              viewport={{ once: true }}
              transition={{ duration: 1.2, ease: "easeOut" }}
              className="relative hidden lg:block perspective-1000 lg:col-span-1 z-0 origin-left"
            >
              <div className="rounded-2xl overflow-hidden border border-white/5 shadow-2xl bg-zinc-950 aspect-video relative group transition-transform duration-500 hover:scale-105">
                {/* Mock Dark UI UI */}
                <div className="p-4 bg-zinc-900 border-b border-white/5 flex items-center gap-2">
                  <div className="flex gap-1.5">
                    <div className="w-2.5 h-2.5 rounded-full bg-red-400/50" />
                    <div className="w-2.5 h-2.5 rounded-full bg-yellow-400/50" />
                    <div className="w-2.5 h-2.5 rounded-full bg-lime-400/50" />
                  </div>
                </div>
                <div className="p-6 bg-zinc-950 h-full">
                  <div className="flex gap-4 mb-6">
                    <div className="h-16 flex-1 rounded-xl bg-white/5 animate-pulse" />
                    <div className="h-16 flex-1 rounded-xl bg-white/5 animate-pulse" />
                  </div>
                  <div className="h-32 rounded-xl bg-white/5 animate-pulse w-full border border-dashed border-white/10" />
                </div>
                <div className="absolute inset-0 bg-gradient-to-tl from-lime-400/10 via-transparent to-transparent pointer-events-none" />
                <div className="absolute bottom-4 right-4 glass px-4 py-1.5 rounded-full text-[10px] font-black text-white tracking-tighter uppercase">MODO ESCURO</div>
              </div>
            </motion.div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {features.map((feature, index) => (
              <motion.div
                ...
                viewport={{ once: true }}
                transition={{ delay: index * 0.1 }}
                className="group p-8 rounded-3xl bg-white border border-zinc-100 shadow-sm hover:shadow-xl transition-all cursor-default"
              >
                <div className={`w-14 h-14 rounded-2xl bg-gradient-to-br ${feature.color} flex items-center justify-center mb-6 transform group-hover:scale-110 group-hover:rotate-3 transition-all`}>
      ...
      <section className="py-32 px-6">
        <div className="max-w-5xl mx-auto rounded-[40px] bg-gradient-to-br from-lime-600 to-lime-800 p-12 md:p-24 text-center relative overflow-hidden">
          <div className="absolute inset-0 bg-[url('https://www.transparenttextures.com/patterns/carbon-fibre.png')] opacity-20" />
          <div className="relative z-10">
            <h2 className="text-4xl md:text-6xl font-black text-white mb-8 leading-tight tracking-tighter">
              Cada clube tem uma realidade. Nossa operação se adapta à sua.
            </h2>
            <p className="text-lime-50/80 text-lg md:text-xl max-w-2xl mx-auto mb-10">
              Converse com a TIMES PRO e veja como podemos estruturar a gestão administrativa, financeira e digital do seu clube.
            </p>
            <button className="px-10 py-5 bg-zinc-950 text-white rounded-full font-black text-lg hover:bg-zinc-900 shadow-2xl transition-all transform hover:scale-105 active:scale-95 flex items-center justify-center gap-3 mx-auto">
              <MessageCircle className="w-6 h-6 fill-lime-400 text-lime-400" />
